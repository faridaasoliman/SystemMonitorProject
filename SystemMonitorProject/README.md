...
- Open `http://localhost:8000` in your browser. The page polls `/metrics` every 5s; the server runs `collector/collect-all.sh` on each poll and reads the latest values from `logs/*.log`. The run is recorded to `logs/cron.log`.
